...
export const HEADER_TEXT_COLOR_KEY = "HEADER_TEXT_COLOR"
export const HEADER_ACCENT_COLOR_KEY = "HEADER_ACCENT_COLOR"
export const HEADER_LAYOUT_STYLE_KEY = "HEADER_LAYOUT_STYLE"

export const HEADER_LAYOUT_STYLE_VALUES = ["centered", "left"] as const
export type HeaderLayoutStyle = (typeof HEADER_LAYOUT_STYLE_VALUES)[number]

/** Header görünümü (renkler + düzen) ayarları — HomeHeader.tsx tarafından okunur. */
export const HEADER_APPEARANCE_SETTING_KEYS = [
  HEADER_BG_COLOR_KEY,
...
export type StoreSettingGroup = "site" | "shipping" | "general" | "footer" | "header"
...
type RegistryStringEntry = {
  ...
  dbType: "string"
  defaultValue: string
  /** plain: serbest metin; relativePath: / ile başlayan güvenli yol; titleTemplate: içinde %s zorunlu; httpUrl: boş veya geçerli http(s) URL; hexColor: #RRGGBB veya #RGB; enum: enumValues içindeki değerlerden biri */
  stringKind?: "plain" | "relativePath" | "titleTemplate" | "httpUrl" | "hexColor" | "enum"
  /** stringKind "enum" olduğunda izin verilen değerler. */
...
export const STORE_SETTINGS_REGISTRY: StoreSettingsRegistryEntry[] = [
  ...
  {
    key: HEADER_BG_COLOR_KEY,
    group: "header",
    valueType: "string",
    label: "Header arka plan rengi",
    description: "Üst menü şeridinin arka plan rengi. #RRGGBB formatında girin.",
    maxLength: 7,
    dbType: "string",
    defaultValue: "#FBF8F2",
    stringKind: "hexColor",
  },
  {
    ...
    description: "Logo çevresi hariç, menü ve ikon metinlerinin rengi. #RRGGBB formatında girin.",
    maxLength: 7,
    dbType: "string",
    defaultValue: "#27231F",
    stringKind: "hexColor",
  ...
  {
    key: HEADER_ACCENT_COLOR_KEY,
    group: "header",
    valueType: "string",
    label: "Header vurgu rengi",
    description: "Hover ve aktif durumlardaki ikon/link rengi. #RRGGBB formatında girin.",
    maxLength: 7,
    dbType: "string",
    defaultValue: "#B99754",
    stringKind: "hexColor",
  ...
  {
    key: HEADER_LAYOUT_STYLE_KEY,
    group: "header",
    valueType: "string",
    label: "Header düzeni",
    description: "Logo ve arama çubuğunun yerleşimi.",
    maxLength: 20,
    dbType: "string",
    defaultValue: "centered",
    stringKind: "enum",
    enumValues: HEADER_LAYOUT_STYLE_VALUES,
  },
]
...
export function parseRegistryUpdate(
  ...
  if (kind === "relativePath") {
    ...
    val = r.data
  } else if (kind === "hexColor") {
    const r = z
      .string()
      .regex(/^#([0-9A-Fa-f]{6}|[0-9A-Fa-f]{3})$/, "Renk #RRGGBB veya #RGB formatında olmalı.")
      .safeParse(val)
    if (!r.success) {
      return { error: r.error.flatten().formErrors.join(", ") || "Geçersiz renk." }
    }
    val = r.data.toUpperCase()
  } else if (kind === "enum") {
    const allowed = entry.enumValues ?? []
    if (!allowed.includes(val)) {
      return { error: `Değer şunlardan biri olmalı: ${allowed.join(", ")}` }
    }
  }
